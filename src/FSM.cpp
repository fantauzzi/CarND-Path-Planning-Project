...
FSM_State * KeepLane::getNextState(const Car & theCar, const std::vector<CarSensorData> theCars) {
	...
	if (consider_lane_change) {
		...
		for (auto the_lane: lanes) {
			...
			auto preceding = findClosestInLane({car.s, car.d }, cars, car.getLane(), true, ConfigParams::lane_width);
			...
			auto following = findClosestInLane({car.s, car.d }, cars, car.getLane(), false, ConfigParams::lane_width);
			...
			if (viable && (preceding_i<0 || (preceding_i>=0 && cars[preceding_i].getSpeed() > new_lane_speed))) {
				new_lane= the_lane;
				new_lane_speed= (preceding_i >= 0)? cars[preceding_i].getSpeed() : ConfigParams::cruise_speed;
			}
